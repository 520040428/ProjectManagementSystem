 ...
 * @description 核心目的是解决Spring Cloud Gateway中请求体只能读取一次的问题
 * InputStream（输入流）的设计遵循**“单向流动”**的原则。当你从一个流中读取数据时，内部的读取指针（或位置标记）会不断向后移动。当数据被读取完毕（到达流末尾，返回 -1）后，指针就停在了末尾。再次调用read()时，会直接返回-1，无法获取其他数据
 *
 * @author canghe
 */
@Component
